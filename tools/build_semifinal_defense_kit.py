        ...
        with zipfile.ZipFile(io.BytesIO(data), mode="r") as archive:
            ...
            for info in infos:
                ...
                if suffix == ".pptx" and (
                    ...
                    or any(folded.startswith(prefix) for prefix in PPTX_FORBIDDEN_PREFIXES)
                ):
...
def _require_tracked(root: Path, relative: str) -> Path:
    stage = _run_git(
        root, "ls-files", "--stage", "--error-unmatch", "--", relative
    )
...
def _validate_public_manifest(
    ...
    *,
    ...
    for entry in entries:
        if not isinstance(entry, dict) or not {"path", "sha256", "size_bytes"}.issubset(
            entry
...
def build(
    *,
    ...
    if not _is_git_ignored(root, output_zip) or not _is_git_ignored(
        root, receipt_path
    ):
    ...
    try:
        ...
        if hashlib.sha256(final_zip_bytes).hexdigest() != receipt[
            "zip_sha256"
