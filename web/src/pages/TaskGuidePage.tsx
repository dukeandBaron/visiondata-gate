...
export function TaskGuidePage() {
  ...
  return <div className="task-guide-page">
    ...
    {!task ? <section className="task-guide-unknown" role="alert"><h2>未找到这个业务任务</h2><p>请选择上方的一项工作。未知链接不会套用默认任务，也不会创建或批准后台工作。</p></section> : <>
      ...
      <details className="task-guide-proof"><summary>决赛核验与试点：这项工作如何形成证据？</summary><p>以下与 2026-09-16 核对的决赛五维、十三项标准使用同一事实源；分值是官方权重，不是本项目预测得分。</p><div className="task-guide-proof__scroll"><table><thead><tr><th scope="col">评审维度</th><th scope="col">要回答的问题</th><th scope="col">本任务需要的证明</th></tr></thead><tbody>{scoringCriteria.map((criterion) => <tr key={criterion.id}><th scope="row">{criterion.name}<small>{criterion.weight}</small></th><td>{criterion.question}</td><td>{task.proof[criterion.id]}<small>{criterion.status} · {criterion.boundary}</small></td></tr>)}</tbody></table></div><p>目前没有客户独立使用、付费验收与跨项目效果数据。应先记录独立完成率、人工有效工作时长和第二项目接入成本；不能把数据检查通过写成产线良品率提升。</p></details>
    </>}
  </div>;
}
