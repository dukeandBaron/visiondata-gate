...
export interface SelectionLearningRequest extends CycleLearningRequest { expected_run_sha256: string; action: 'APPROVE_SANDBOX' | 'REJECT' }
...
export interface LearningRun {
  ...
  selection: { action: 'APPROVE_SANDBOX' | 'REJECT'; actor: string; note: string; evaluation_sha256: string; at: string } | null;
...
export function validateSelectionLearningRequest(value: unknown): SelectionLearningRequest {
  const row = shape(value, [...cycleRequestKeys, 'expected_run_sha256', 'action']);
  authorizeCycle(row); sha(row.expected_run_sha256); member(row.action, ['APPROVE_SANDBOX', 'REJECT']);
...
export async function validateLearningRun(value: unknown, scope: LearningScope, cycleId: string, runId?: string, etag?: string | null): Promise<LearningRun> {
  ...
  if (row.selection !== null) {
    const selection = shape(row.selection, ['action', 'actor', 'note', 'evaluation_sha256', 'at']); member(selection.action, ['APPROVE_SANDBOX', 'REJECT']);
    ...
    if (selection.action === 'APPROVE_SANDBOX') ensure(object(row.evaluation).decision === 'ELIGIBLE');
  }
