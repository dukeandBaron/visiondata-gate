...
旧 VisA 回执绑定旧版核心源码。本次合同/工具实现变化后，其当前源码绑定校验会失败，界面必须保持 `PUBLIC_PROXY_UNAVAILABLE_WITH_HISTORICAL_OFFLINE_EVIDENCE`，不能继续当作新版本的有效成绩。原回执和分母未重写；新的 VisA 复测需在原数据与协议可用时独立执行。
