...
VisionData Gate 的产品形态是本地或内网部署的工业工作台：React/Tauri 负责桌面界面，Spring Boot WebFlux 提供环回网关，FastAPI 负责案件、证据、Agent 和策略治理。它不是互联网 SaaS。本地已经生成内嵌 Java 与 Python 运行时的 unsigned NSIS test installer，但干净 Windows 安装/卸载、签名和正式发布均未验证。普通安装用户不需要 Python、Java、Node.js 或 Rust；以下依赖只适用于源码开发。

当前发布状态见 [PROJECT_STATUS.md](PROJECT_STATUS.md)。本地运行成功不等于 RC3 已冻结、官方已提交或生产可以放行。

## 环境

无需 PowerShell 的 Python/React 源码入口见 [跨平台 Quickstart](CROSS_PLATFORM_QUICKSTART.md)。React 是主要操作界面；Tauri、历史 Streamlit 和 Reviewer 服务的不同职责见 [界面选择](INTERFACE_SUPPORT.md)。下面保留既有 Windows 部署配置，不代表 Linux/macOS 原生安装包已经验证。
...
如果只需启动一侧，可分别运行 `.\run_api.ps1` 和 `.\run_web.ps1`，但 API 必须具有显式会话令牌；单独设置 `X-Actor-User-Id` 不能绕过认证。普通使用优先由工作台启动器创建和交接会话，不把令牌打印在日志或写进源码。接口只监听本机；本地会话认证不等同生产 IAM。macOS/Linux 的 Web 源码入口与桌面包未完成边界见 [跨平台 Quickstart](CROSS_PLATFORM_QUICKSTART.md)。
