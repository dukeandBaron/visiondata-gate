...
- **Normality 热图**：后端保存像素工件，当前 HTTP 合同只公开摘要、尺寸与工件 ID；界面不伪造预览。
- **Normality 人工反馈**：当前界面仅页面内存草稿，状态 `LOCAL_DRAFT_NOT_SUBMITTED`；未写入服务端，不进入训练。
- **统一防遗忘门禁**：参考学习与持续学习评测合同不能直接外推到每个 YOLO/Normality 产品任务。
- **TTT、RL、VLM 预标注、主动学习、自动 Mask**：不能列为已贯通的产品能力；TTT 明确关闭。
