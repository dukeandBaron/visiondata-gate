...
def build_reviewer_readiness(
    ...
    dimensions = [
        ...
        dimension(
            ...
            verified_now=[
                "已生成 SBOM 与第三方依赖许可清单，提交包会排除密钥和运行痕迹",
            ],
            gaps=[
                "顶层 LICENSE/NOTICE 权利主体确认仍待外部完成",
                "尚无公开仓库、外部贡献或正式开源发布回执",
            ],
            reviewer_check=[
                "核对顶层许可证、NOTICE、依赖许可证与可再分发范围",
                "如申报开源贡献，要求公开仓库链接与版本 tag",
            ],
            local_basis="SBOM/license inventory present, rights confirmation incomplete",
        ),
    ]
    ...
    *,
    ...
    claims = [
        ...
        },
        {
            ...
            "statement": (
                "逐工具消融回执显示：删除任一观测工具不会让冻结策略变得更宽松，"
                "并明确暴露新增失败规则与丢失的 finding 证据。"
            ),
...
def build_claim_scope_receipt(trace: RuntimeTrace) -> dict[str, Any]:
...
def write_proof_artifacts(
    ...
    *,
    ...
    """Write observability and proof artifacts and return their hashes."""
    ...
    claim_scope = build_claim_scope_receipt(trace)
    ...
    ablation_bytes: bytes | None = None
    if replay_context is not None:
        ...
        missing_replay = required_replay_keys - set(replay_context)
        if missing_replay:
            raise ValueError(
                "replay_context is missing required keys: "
                + ", ".join(sorted(missing_replay))
            )
        replay = build_tool_replay_receipt(
            trace,
            initial_root=replay_context["initial_root"],
            initial_manifest=replay_context["initial_manifest"],
            repaired_root=replay_context["repaired_root"],
            repaired_manifest=replay_context["repaired_manifest"],
            contract=replay_context["contract"],
            initial=initial,
            repaired=repaired,
        )
        replay_path = root / "tool_replay_receipt.json"
        replay_bytes = canonical_json_bytes(replay)
        replay_path.write_bytes(replay_bytes)
        ablation = build_tool_ablation_receipt(
            trace,
            initial_manifest=replay_context["initial_manifest"],
            repaired_manifest=replay_context["repaired_manifest"],
            contract=replay_context["contract"],
        ...
        ablation_path = root / "tool_ablation_receipt.json"
        ablation_bytes = canonical_json_bytes(ablation)
        ablation_path.write_bytes(ablation_bytes)
    ...
    if replay_path is not None:
        proof_paths["tool_replay_receipt"] = replay_path
    if ablation_path is not None:
        proof_paths["tool_ablation_receipt"] = ablation_path
    ...
    return {
        ...
        **(
            {"tool_replay_receipt.json": sha256_bytes(replay_bytes)}
            if replay_bytes is not None
            else {}
        ),
        **(
            {"tool_ablation_receipt.json": sha256_bytes(ablation_bytes)}
            if ablation_bytes is not None
            else {}
        ),
