"""Historical RC1 public-release bundle and immutable replay validator.
...
This module preserves the 2026-08-16 RC1 contract for immutable replay. It is
not the active RC3 release policy; current candidates are governed by
``release_attestation.py``, ``release_evidence.py``, and ``package.py``. It
never infers customer, factory, production, or hosted AgentTeams status from
historical artifacts and refuses public JSON containing local absolute paths
or private-evidence markers.
"""
...
LEGACY_RC1_RELEASE_ID = "vdg-20260816-rc1"
# Backward-compatible library alias. Current RC3 tooling must not use this as
# an implicit release target.
DEFAULT_RELEASE_ID = LEGACY_RC1_RELEASE_ID
