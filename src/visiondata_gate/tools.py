...
ToolResult = tuple[list[Finding], ToolTrace, dict[str, MetricValue]]
ToolRunner = Callable[..., object]
...
_REGISTERED_TRACE_ADAPTERS = {
    ...
    "external-readonly-geometry-v1",
}


def tool_contract_catalog(
    *, include_optional: bool = False, include_geometry: bool = False
) -> list[ToolContract]:
    ...
    if include_geometry:
        # Keep the geometry adapter out of the frozen runtime catalog.  It is
        # a separately invoked external-readonly evidence layer, but its trace
        # can still be validated by the same contract machinery.
        from .geometry_consistency import geometry_tool_contract

        contracts["geometry_consistency"] = geometry_tool_contract()
        names.append("geometry_consistency")
    return [contracts[name] for name in names]


def tool_contract_digest(
    tool_name: str, *, include_optional: bool = False, include_geometry: bool = False
) -> str:
    """Return the canonical digest that binds a trace to its tool contract."""

    for contract in tool_contract_catalog(
        include_optional=include_optional, include_geometry=include_geometry
    ):
...
def validate_tool_contract_trace(
    ...
    *,
    ...
    if (
        trace.adapter == "external-readonly-geometry-v1"
        and trace.tool != "geometry_consistency"
    ):
        return "geometry adapter cannot be used for a core tool"
    if (
        trace.tool == "geometry_consistency"
        and trace.adapter != "external-readonly-geometry-v1"
    ):
        return "geometry_consistency requires the geometry adapter"
...
def build_batch_fingerprint(
    batch_root: str | Path,
    manifest: BatchManifest | str | Path | Mapping[str, Any],
) -> str:
    """Hash one validated batch snapshot for reuse by parallel gateway calls.

    The returned value is only an expected request binding.  Each local Worker
    still recomputes its own fingerprint, and the gateway rejects the response
    if the bytes changed after this snapshot was taken.  Reuse therefore
    removes duplicate gateway-side scans without weakening stale-response
    detection.
    """

    root = _validated_root(batch_root)
    validated_manifest = _load_model(manifest, BatchManifest)
    return _batch_fingerprint(root, validated_manifest)


def _validated_batch_fingerprint(value: str | None) -> str | None:
    if value is None:
        return None
    ...
    if len(normalized) != 64 or any(
        ...
        raise ValueError("batch fingerprint must be a 64-character SHA-256")
    return normalized
...
def _prepare_tool_request(
    ...
    *,
...
) -> tuple[Path, BatchManifest, BatchContract, dict[str, Any], str]:
    catalog = {
        name for name, _prefix, _function, _permission in _catalog(include_optional)
    }
    ...
    active_batch_fingerprint = _validated_batch_fingerprint(batch_fingerprint)
    if active_batch_fingerprint is None:
        active_batch_fingerprint = _batch_fingerprint(root, validated_manifest)
    input_sha256 = _canonical_sha256(
        {
            "tool": tool_name,
            "batch_sha256": active_batch_fingerprint,
...
def build_tool_request_sha256(
    ...
    *,
    ...
    """Build the canonical digest for the exact request a Worker is handling."""

    return _prepare_tool_request(
        tool_name,
        batch_root,
        manifest,
        contract,
...
def _result_digest_matches(
    ...
    *,
    ...
    """Verify the legacy raw-metric digest without changing its evidence contract.

    ``run_tool`` hashes raw tool metrics and prefixes keys only for the returned
    aggregate payload.  A raw key may already carry the prefix, so the inverse
    mapping is mildly ambiguous.  The local catalog has at most ten metric keys;
    bounded candidate enumeration preserves the existing digest contract while
    still rejecting changed values or findings.
    """
    ...
    for key in returned_keys:
        ...
        choices.append((stripped, key) if stripped != key else (key,))
    if sum(len(item) > 1 for item in choices) > 12:
        return False

    finding_payload = [item.model_dump(mode="json") for item in findings]
    for raw_keys in product(*choices):
        if len(raw_keys) != len(set(raw_keys)):
            continue
        raw_metrics = {
            raw_key: metrics[returned_key]
            for returned_key, raw_key in zip(returned_keys, raw_keys, strict=True)
        }
        observed = _canonical_sha256(
            {"findings": finding_payload, "metrics": raw_metrics}
        )
        if observed == trace.result_sha256:
            return True
    return False
...
def validate_tool_response(
    ...
    *,
    ...
    """Return a bounded error when a runtime tool response is not trustworthy."""

    if not isinstance(response, tuple) or len(response) != 3:
        return "malformed payload: expected (Finding[], ToolTrace, metrics) tuple"
    findings, trace, metrics = response
    if not isinstance(findings, list) or any(
        not isinstance(item, Finding) for item in findings
    ):
        return "malformed payload: findings must be a typed Finding list"
    if not isinstance(trace, ToolTrace):
        return "malformed payload: trace must be a typed ToolTrace"
    if not isinstance(metrics, dict) or any(
        ...
        return "malformed payload: metrics must be finite typed scalar values"
    if trace.tool != tool_name:
        return f"target mismatch: requested {tool_name}, received {trace.tool}"
    if trace.status != "ok" or trace.error is not None:
        return "status/error mismatch: successful response requires status=ok and error=null"

    catalog = tool_catalog(include_optional=include_optional)
    expected_sequence = next(
        (int(item["sequence"]) for item in catalog if item["name"] == tool_name),
        None,
    )
    if expected_sequence is None:
        return f"unregistered runtime tool: {tool_name}"
    if trace.sequence != expected_sequence:
        return (
            f"sequence mismatch for {tool_name}: "
            f"{trace.sequence} != {expected_sequence}"
        )
    ...
    active_request_sha256 = expected_request_sha256 or build_tool_request_sha256(
        tool_name,
        batch_root,
        manifest,
        contract,
    ...
    if trace.input_sha256 != active_request_sha256:
        return f"stale response: request digest mismatch for {tool_name}"
    validated_contract = _load_model(contract, BatchContract)
    expected_parameters = _trace_parameters(tool_name, validated_contract)
    if trace.parameters != expected_parameters:
        return f"request parameter drift for {tool_name}"

    typed_findings = cast(list[Finding], findings)
    finding_ids = [item.finding_id for item in typed_findings]
    if len(finding_ids) != len(set(finding_ids)):
        return f"duplicate finding IDs returned by {tool_name}"
    if any(item.tool != tool_name for item in typed_findings):
        return f"finding target mismatch for {tool_name}"
    if trace.finding_ids != finding_ids:
        return f"finding ID linkage mismatch for {tool_name}"

    metric_prefix = next(
        str(item["metric_prefix"]) for item in catalog if item["name"] == tool_name
    )
    typed_metrics = cast(dict[str, MetricValue], metrics)
    if not _result_digest_matches(
        typed_findings,
        trace,
        typed_metrics,
        metric_prefix=metric_prefix,
    ):
        return f"result digest mismatch for {tool_name}"
    return None


def build_tool_error_trace(
    tool_name: str,
    message: str,
    *,
    include_optional: bool = False,
    ...
    """Create a contract-bound typed error receipt owned by the local gateway."""

    catalog = tool_catalog(include_optional=include_optional)
    item = next((entry for entry in catalog if entry["name"] == tool_name), None)
    if item is None:
        raise ValueError(f"tool is not whitelisted: {tool_name}")
    status = "skipped" if skipped else "error"
    bounded_message = (message.strip() or "unspecified tool failure")[:500]
    result_sha256 = _canonical_sha256(
        {
            "tool": tool_name,
            "status": status,
    ...
    return ToolTrace(
        ...
        tool=tool_name,
        status=status,
        input_sha256=input_sha256 or result_sha256,
        parameters={},
        result_sha256=result_sha256,
        finding_ids=[],
        error=bounded_message,
...
def execute_tool_gateway(
    ...
    *,
    ...
    """Run one adapter and fail closed on exceptions or untrusted responses."""

    request_sha256 = build_tool_request_sha256(
        tool_name,
        batch_root,
        manifest,
        contract,
    ...
    if response_error:
        ...
        return [], trace, {}
    return cast(ToolResult, response)
