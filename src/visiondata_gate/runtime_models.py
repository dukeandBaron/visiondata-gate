...
class ModelBackendKind(str, Enum):
    ...
    LONGCAT_OPENAI_COMPATIBLE = "longcat_openai_compatible"
...
class RuntimeConfig(RuntimeModel):
    ...
    remote_endpoint_hosts: list[str] = Field(default_factory=list)
    backend_execution_mode: Literal["contract_test", "real"] = "contract_test"
    model_timeout_seconds: float = Field(default=30.0, ge=2.0, le=120.0)
    model_max_retries: int = Field(default=1, ge=0, le=3)
    model_backoff_seconds: float = Field(default=0.05, ge=0.0, le=10.0)
    model_circuit_failure_threshold: int = Field(default=2, ge=1, le=10)
    model_circuit_recovery_seconds: float = Field(default=5.0, ge=0.01, le=300.0)
    ...
    @classmethod
    def unique_endpoint_hosts(cls, values: list[str]) -> list[str]:
        normalized = [value.strip().casefold().rstrip(".") for value in values]
        if any(not value for value in normalized):
            raise ValueError("remote_endpoint_hosts cannot contain blank values")
        if len(normalized) != len(set(normalized)):
            raise ValueError("remote_endpoint_hosts must be unique")
        return normalized
