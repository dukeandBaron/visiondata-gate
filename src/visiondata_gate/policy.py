...
if TYPE_CHECKING:
    from .rulepack import RulePackRuntimeBinding
...
def _finding_action(
    ...
    if runtime_rulepack is not None:
        # An explicitly activated pack is authoritative for known codes.  An
        # unmatched code never falls through to fuzzy keyword inference.
        return runtime_rulepack.action_by_finding_code.get(code, "INVESTIGATE")
...
def _finding_work_order(
    ...
    *,
    ...
    action: WorkOrderAction = (
        "INVESTIGATE"
        if force_investigate
        else _finding_action(finding, runtime_rulepack)
    )
    payload = {
        "action": action,
    ...
    if runtime_rulepack is not None:
        rule_id = runtime_rulepack.rule_id_by_finding_code.get(
            finding.code.upper(), "UNMATCHED_FAIL_CLOSED"
        )
        rulepack_binding = _work_order_rulepack_binding(
            runtime_rulepack,
            rule_id=rule_id,
        )
        payload.update(rulepack_binding)
        replacement_requirements.update(rulepack_binding)
    return WorkOrder(
...
def _work_order_rulepack_binding(
    runtime_rulepack: RulePackRuntimeBinding,
    *,
    rule_id: str,
) -> dict[str, str]:
    return {
        "rule_pack_id": runtime_rulepack.pack_id,
        "rule_pack_version": runtime_rulepack.pack_version,
        "rule_pack_source_identity": runtime_rulepack.source_identity,
        "rule_pack_source_sha256": runtime_rulepack.source_sha256,
        "rule_pack_semantic_sha256": runtime_rulepack.semantic_sha256,
        "rule_pack_binding_sha256": runtime_rulepack.binding_sha256,
        "rule_id": rule_id,
    }


def _trace_work_order(
    trace: ToolTrace,
    runtime_rulepack: RulePackRuntimeBinding | None = None,
) -> WorkOrder:
    ...
    replacement_requirements: dict[str, Any] = {
        "tool": trace.tool,
    ...
    if runtime_rulepack is not None:
        finding_code = (
            ...
            else "FOLLOWUP_BUDGET_EXHAUSTED"
        )
        rule_id = runtime_rulepack.rule_id_by_finding_code.get(
            finding_code,
...
def _missing_trace_work_order(
    runtime_rulepack: RulePackRuntimeBinding | None = None,
) -> WorkOrder:
    payload = {"action": "INVESTIGATE", "reason_codes": ["NO_TOOL_TRACE"]}
    replacement_requirements: dict[str, Any] = {
        "required": "at least one completed tool trace"
    }
    if runtime_rulepack is not None:
        rulepack_binding = _work_order_rulepack_binding(
            runtime_rulepack,
            rule_id="UNMATCHED_FAIL_CLOSED",
        )
        payload.update(rulepack_binding)
        replacement_requirements.update(rulepack_binding)
    return WorkOrder(
...
def _build_core_work_orders(
    ...
    work_orders.extend(
        _trace_work_order(trace, runtime_rulepack) for trace in incomplete_traces
    )
    if not trace_list:
        work_orders.append(_missing_trace_work_order(runtime_rulepack))

    unsupported_ids = {finding.finding_id for finding in unsupported}
    work_orders.extend(
        _finding_work_order(
            finding,
            force_investigate=finding.finding_id in unsupported_ids,
...
def _build_counterfactual_rule_checks(
    ...
    for finding in candidates:
        ...
        candidate_work_orders, candidate_incomplete_traces, block_release = (
            _build_core_work_orders(perturbed, trace_list, runtime_rulepack)
        )
...
def _build_counterfactual_tool_checks(
    ...
    for probe_id, candidate_trace in candidates:
        candidate_work_orders, _, block_release = _build_core_work_orders(
            finding_list, candidate_trace, runtime_rulepack
        )
        candidate_decision, _ = _derive_gate_decision(
            candidate_work_orders, block_release
        )
...
def apply_policy(
    ...
    *,
    ...
    if runtime_rulepack is not None:
        from .rulepack import verify_rule_pack_runtime_binding

        runtime_rulepack = verify_rule_pack_runtime_binding(runtime_rulepack)
    ...
    work_orders, _, block_release = _build_core_work_orders(
        finding_list, trace_list, runtime_rulepack
    )
    ...
    if runtime_rulepack is not None:
        rule_checks.insert(
            0,
            RuleCheck(
                check_id="RC-RULEPACK-RUNTIME-BINDING",
                status=RuleCheckResult.PASS,
                detail=(
                    f"Runtime used {runtime_rulepack.pack_id}@"
                    f"{runtime_rulepack.pack_version}; unmatched finding codes "
                    "fail closed to INVESTIGATE."
                ),
                related_refs=[
                    f"sha256:{runtime_rulepack.binding_sha256}",
                    f"rulepack:{runtime_rulepack.pack_id}",
                ],
            ),
        )
    if decision is GateDecision.PASS:
        required_check_ids = required_gate_check_ids(scenario_profile)
        failed_required_check_ids = sorted(
            check.check_id
            for check in rule_checks
            if check.status is RuleCheckResult.FAIL
            and check.check_id in required_check_ids
        )
        invariant_receipt = build_runtime_invariant_receipt(
            RuntimeInvariantContext(
                action=RuntimeAction.GATE_PASS,
                scenario_profile=scenario_profile,
                input_sha256=digest,
                failed_required_rule_check_ids=failed_required_check_ids,
        ...
        failed_invariants = [
            item.invariant_id
            for item in invariant_receipt.outcomes
            if item.status is RuntimeInvariantStatus.FAIL
        ]
        rule_checks.append(
            RuleCheck(
                check_id="RC-RUNTIME-INVARIANT-GUARD",
                status=(
                    RuleCheckResult.PASS
                    if invariant_receipt.allowed
                    else RuleCheckResult.FAIL
                ),
                detail=(
                    "Executable runtime invariants allow the sandbox Gate PASS transition."
                    if invariant_receipt.allowed
                    else (
                        "Executable runtime invariants blocked Gate PASS: "
                        + ", ".join(failed_invariants)
                        + "."
                    )
                ),
                related_refs=[
                    f"runtime-invariant-receipt:sha256:{invariant_receipt.receipt_sha256}",
                    *(f"rule-check:{item}" for item in failed_required_check_ids),
                ],
            )
        )
        if not invariant_receipt.allowed:
            decision = GateDecision.DEFER
            decision_reason = (
                f"DEFER: scenario profile {scenario_profile.value} failed mandatory "
                "PASS-level runtime invariants ("
                + ", ".join(failed_required_check_ids)
                + ")."
            )
    ...
    return GateResult(
        ...
        input_sha256=digest,
        policy_version=(
            f"{contract.policy_version}+{runtime_rulepack.pack_id}@"
            f"{runtime_rulepack.pack_version}"
            if runtime_rulepack is not None
            else contract.policy_version
        ),
        decision=decision,
