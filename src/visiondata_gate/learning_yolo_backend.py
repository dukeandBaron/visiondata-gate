...
def _command(executable: Path, mode: str, request: Path, result: Path) -> list[str]:
    ...
    return [str(executable), "-I", "-S", str(Path(__file__).resolve()),
...
def probe_vision_runtime(
    ...
    *, import_check: bool = False,
    ...
    return _probe_runtime(executable, expected_executable_sha256, work_root,
                          import_check=import_check, timeout=45 if import_check else 20)
...
def _child_train(request: dict) -> dict:
    ...
    train_options = {"data": str(dataset / "data.yaml"), "device": "cpu",
                     "epochs": config.epochs, "imgsz": config.imgsz,
                     "batch": config.batch, "workers": 0, "amp": False,
                     "seed": config.seed, "deterministic": True, "pretrained": False,
                     "plots": False, "save": True, "val": True, "cache": False,
                     "project": str(output), "name": "training", "exist_ok": False,
                     "optimizer": "SGD", "lr0": 0.001, "momentum": 0.9,
                     "mosaic": 0.0, "mixup": 0.0, "copy_paste": 0.0,
                     "auto_augment": None, "close_mosaic": 0, "patience": 0,
