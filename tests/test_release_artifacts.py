...
def test_semifinal_rc3_video_qa_binds_current_workbench_demo() -> None:
    qa_path = (
        PROJECT_ROOT / "deliverables" / "_qa" / "semifinal_rc3_video_qa_20260831.json"
    )
    video_member = "deliverables/VisionDataGate_GOAI_Semifinal_RC3_20260831.mp4"
    contact_sheet_member = (
        "deliverables/_qa/semifinal_rc3_video_contact_sheet_20260831.png"
    )
    video_sha256 = "bb97a0ca947079eaac6d15451e1c585965637c55e9cbd67b0726294eabf0273e"
    ...
    assert qa["video_file"] == video_member
    assert qa["sha256"] == video_sha256
    assert qa["file_size_bytes"] == 3_630_089
    assert video_path.is_file()
    assert video_path.stat().st_size == qa["file_size_bytes"]
    assert _sha256(video_path) == video_sha256
    assert video_path.read_bytes()[4:8] == b"ftyp"

    assert qa["contact_sheet"] == {
        "file_size_bytes": 455_044,
        "path": contact_sheet_member,
        "sha256": contact_sheet_sha256,
    }
    assert contact_sheet_path.is_file()
    assert contact_sheet_path.stat().st_size == 455_044
    assert _sha256(contact_sheet_path) == contact_sheet_sha256
    assert contact_sheet_path.read_bytes()[:8] == b"\x89PNG\r\n\x1a\n"

    assert qa["resolution"] == [1920, 1080]
    assert qa["fps"] == 30.0
    assert 89.8 <= qa["duration_seconds"] <= 90.0
    assert qa["decode"] == {
        "decoded_frame_count": 2696,
        "errors": [],
        "exit_code": 0,
        "sample_timestamps_seconds": [5, 17, 30, 43, 56, 70, 84],
        "sampled_frame_count": 7,
    }
    assert qa["audio"]["present"] is True
    assert qa["audio"]["codec"] == "aac-lc"
    assert qa["audio"]["mean_volume_db"] > -30.0
    assert qa["checks"] and all(qa["checks"].values())
    assert qa["anonymity"] == {
        "contains_absolute_local_paths_in_metadata": False,
        "forbidden_metadata_hits": [],
    }
    assert qa["source_modes"] == [
        "REAL_LOCAL_UI_CAPTURE",
        "PRESENTATION_DERIVED_MOTION_GRAPHICS",
    ]
    assert qa["boundaries"] == {
