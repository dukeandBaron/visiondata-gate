...
def test_reviewer_audit_separates_official_competitor_and_local_layers(
    ...
    assert payload["source_layers"]["official_or_mentor"]
...
def test_tool_contract_snapshot_is_complete_and_run_bound(tmp_path) -> None:
    run = run_agentic_demo(
        tmp_path / "run",
        seed=20260813,
        config=RuntimeConfig(scenario_profile=ScenarioProfile.INDUSTRIAL),
...
def test_proof_index_includes_reviewer_comparison_artifacts(tmp_path) -> None:
    run = run_agentic_demo(
        tmp_path / "run",
        seed=20260814,
...
def test_runtime_contract_audit_checks_the_executable_core(tmp_path) -> None:
    run = run_agentic_demo(
        tmp_path / "run",
        seed=20260815,
        config=RuntimeConfig(scenario_profile=ScenarioProfile.INDUSTRIAL),
...
def test_context_transfer_receipt_marks_missing_worker_as_deferred(tmp_path) -> None:
    run = run_agentic_demo(
        tmp_path / "restricted",
        seed=20260818,
        config=RuntimeConfig(
...
def test_tool_replay_receipt_matches_both_gate_phases(tmp_path) -> None:
    run = run_agentic_demo(
        tmp_path / "run",
        seed=20260817,
        config=RuntimeConfig(scenario_profile=ScenarioProfile.INDUSTRIAL),
    ...
    regenerated = build_tool_replay_receipt(
        ...
        initial_manifest=BatchManifest.model_validate_json(
    ...
    # The persisted receipt is the authoritative shape; this assertion only
    # verifies that replay artifacts are present and structurally complete.
    assert regenerated["schema_version"] == payload["schema_version"]


def test_tool_ablation_receipt_is_fail_closed_and_run_bound(tmp_path) -> None:
    run = run_agentic_demo(
        tmp_path / "run",
        seed=20260819,
        config=RuntimeConfig(scenario_profile=ScenarioProfile.INDUSTRIAL),
...
def test_skill_qualification_rejects_version_tampering(tmp_path) -> None:
    run = run_agentic_demo(
        tmp_path / "run",
        seed=20260820,
        config=RuntimeConfig(scenario_profile=ScenarioProfile.INDUSTRIAL),
