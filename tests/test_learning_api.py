...
def test_learning_routes_use_real_authentication_and_etag(learning_input):
    ...
    payload = {
        "request_key": "api-learning-create-001",
        "expected_preflight_sha256": preflight["receipt_sha256"],
        "groups": groups,
        "operator_attests_training_authorized": True,
    ...
    outsider = client.post(
        endpoint, headers={"X-Actor-User-Id": "outsider"}, json=payload
    )
    ...
    assert (
        client.get(
            f"/v1/learning-cycles/{cycle['cycle_id']}",
            headers={"X-Actor-User-Id": "outsider"},
        ).status_code
        == 404
    )
    invalid = client.post(
        endpoint, headers=HEADERS, json=payload | {"command": "arbitrary command"}
    )
    assert invalid.status_code == 422


def test_learning_http_real_train_feedback_selection_rollback_and_finalize(
    learning_input,
):
    ...
    listing = client.get(
        f"/v1/projects/{cycle['project_id']}/learning-cycles", headers=HEADERS
    )
    assert listing.status_code == 200
    assert [item["cycle_id"] for item in listing.json()] == [cycle["cycle_id"]]
    assert listing.headers["cache-control"] == "private, no-store"
    initial = _sealed(
        client.get(f"/v1/learning-models/{cycle['initial_model_id']}", headers=HEADERS)
    )
    ...
    assert (
        _sealed(client.post(cycle_path + "/rounds", headers=HEADERS, json=execute))
        == run
    )
    assert (
        _sealed(client.get(f"/v1/learning-runs/{run['run_id']}", headers=HEADERS))
        == run
    )
    trained = _sealed(
        client.get(f"/v1/learning-models/{run['model_id']}", headers=HEADERS)
    )
    assert (
        trained["model_sha256"]
        == hashlib.sha256(canonical_jcs_bytes(trained["weights"])).hexdigest()
    )
    ...
    assert run["feedback"], (
        "fixture must exercise real prediction-error review through HTTP"
    )
    for item in list(run["feedback"]):
        cycle = _sealed(client.get(cycle_path, headers=HEADERS))
        review = _action(
            cycle, "review-" + item["feedback_id"], classification="HARD_SAMPLE"
        )
        review_path = (
            cycle_path + f"/runs/{run['run_id']}/feedback/{item['feedback_id']}"
        )
        assert (
            client.post(review_path, headers=outsider, json=review).status_code == 404
        )
        run = _sealed(client.post(review_path, headers=HEADERS, json=review))
        reviewed = next(
            row for row in run["feedback"] if row["feedback_id"] == item["feedback_id"]
        )
    ...
    cycle = _sealed(
        client.post(
            ...
            headers=HEADERS,
            json=_action(
                cycle, "api-model-rollback-001", model_id=cycle["initial_model_id"]
            ),
        )
    )
...
def test_learning_auth_reuses_bound_session_and_never_trusts_actor_header(
    ...
    with TestClient(create_app(product)) as client:
        ...
        assert (
            client.post(
                endpoint, headers={"X-VisionData-Session-Token": "wrong"}, json=payload
            ).status_code
            == 401
        )
        assert (
        ...
        cycle = _sealed(
            client.post(
                endpoint, headers={"X-VisionData-Session-Token": token}, json=payload
            ),
            201,
        )
...
@pytest.mark.parametrize(
    "error_type,code",
    [(LearningError, "learning_hold"), (DatasetError, "learning_dataset_hold")],
)
def test_learning_errors_are_fixed_409_without_local_path_echo(
    learning_input, monkeypatch, error_type, code
):
    client, _product, (task_id, preflight, groups) = learning_input
    ...
    def fail(_product):
        error = error_type(
            "E:/synthetic-private-root/secret-file.json must never be echoed"
        )
...
def test_learning_cancel_recover_and_schema_boundaries(learning_input):
    ...
    assert (
        _sealed(client.get(cycle_path, headers=HEADERS))["receipt_sha256"]
    ...
    assert schema["paths"]["/v1/tasks/{task_id}/learning-cycles"]["post"][
        "requestBody"
    ]["content"]["application/json"]["schema"]["$ref"].endswith("/CreateLearningCycle")


def test_learning_route_installation_is_lazy_and_uses_supplied_dependencies(
    monkeypatch,
):
    from visiondata_gate import learning_api
    ...
    with TestClient(app) as client:
        for _ in range(2):
            response = client.get(
                "/v1/projects/project-probe/learning-cycles",
