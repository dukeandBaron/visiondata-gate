...
def test_real_backend_learning_receipts_pass_web_contract(learning_input):
    ...
    assert all(item["status"] == "TRIAGED_NOT_AUTO_INGESTED" for item in run["feedback"])
    ...
    assert operation["request_digest_verification"] == "MATCHED_STORED_VALIDATED_REQUEST"
