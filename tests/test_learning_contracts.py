...
def test_round_feedback_association_is_explicit_not_all_previous_errors():
    ...
    payload = {
        "request_key": "explicit-feedback-001",
        "expected_preflight_sha256": "a" * 64,
        "groups": {"sample": "group"},
        "review_note": "New data does not implicitly resolve every previous error",
        "operator_attests_training_authorized": True,
        "task_id": "task",
        "expected_cycle_sha256": "b" * 64,
    }
...
def test_empty_annotation_is_not_implicitly_a_normal_training_mask():
    ...
    payload = {
        ...
        "review_note": "Explicitly reviewed no foreground in this synthetic image",
        "expected_asset_sha256": "a" * 64,
        "expected_annotation_revision": 0,
        "expected_annotation_sha256": "b" * 64,
        "operator_attests_no_foreground": True,
    }
    assert (
        NormalMaskAttestation.model_validate(payload).operator_attests_no_foreground
        is True
    )
    for changes in (
        {"operator_attests_no_foreground": False},
        {"reviewer_name": ""},
        {"expected_annotation_sha256": "stale"},
    ):
        with pytest.raises(ValidationError):
