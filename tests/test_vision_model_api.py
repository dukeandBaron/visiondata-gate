...
GET_PATHS = (
    ...
    "vision-runtimes",
    "vision-datasets",
    "vision-inference-assets",
    "vision-inference-assets/vision_inference_asset_missing",
    "vision-inferences",
    "vision-inferences/vision_inference_missing",
    "vision-inferences/vision_inference_missing/heatmap",
    "vision-inferences/vision_inference_missing/feedback",
    "vision-ttt-capabilities",
    "vision-ttt-failures",
...
POST_PATHS = (
    "vision-models",
    "vision-model-packs",
    "vision-models/vision_model_missing/sandbox-approval",
    "vision-runtimes",
    "vision-runtimes/runtime-missing/probe",
    "vision-datasets",
    "vision-datasets/from-data-pool",
    "vision-inference-assets",
    "vision-models/vision_model_missing/inferences",
    "vision-models/vision_model_missing/ttt-inferences",
    "vision-inferences/vision_inference_missing/feedback",
    "vision-training-runs",
...
def test_inference_asset_http_route_freezes_bytes_and_redacts_local_path(
    vision_app, tmp_path
):
    app, _product, project_id = vision_app
    image = tmp_path / "private-inspection-frame.png"
    Image.new("RGB", (18, 14), (30, 60, 90)).save(image)
    digest = hashlib.sha256(image.read_bytes()).hexdigest()
    payload = {
        "request_key": "normality-http-asset-register-0001",
        "reviewer_identity": "Named image reviewer",
        "note": "Freeze an authorized local image before inference uses its asset ID",
    ...
    with TestClient(app, client=("127.0.0.1", 50000)) as client:
        ...
        fetched = _sealed(
            client.get(
                base + "/vision-inference-assets/" + asset["asset_id"],
                headers=OWNER_HEADERS,
            )
        )
        listing = _sealed(
            client.get(base + "/vision-inference-assets", headers=OWNER_HEADERS)
        )

    assert fetched == asset
    assert listing["items"] == [asset]
    assert asset["image_sha256"] == digest
...
def test_generic_http_dataset_is_explicitly_unbound_and_not_training_eligible(
    vision_app, tmp_path, monkeypatch
):
    from visiondata_gate import learning_yolo_backend as backend
    from visiondata_gate import local_model_registry as registry

    app, _product, project_id = vision_app
    source = tmp_path / "generic-dataset"
    source.mkdir()
    samples = []
    for index, split in enumerate(("train", "val", "test")):
        image = source / f"{split}.png"
        Image.new("RGB", (16, 16), (20 + index * 40, 70, 100)).save(image)
        samples.append(
            {
                "sample_id": f"generic-{split}",
                "image_path": image.name,
                "image_sha256": hashlib.sha256(image.read_bytes()).hexdigest(),
                "split": split,
                "group_id": f"generic-group-{split}",
                ...
                "reviewer_name": "Synthetic dataset reviewer",
                "reviewed": True,
            }
        )
    manifest = {
        "schema_version": "visiondata-gate.detection-dataset.v1",
        "source_version": "generic-api-fixture-v1",
        "class_names": ["scratch"],
        "samples": samples,
    }
    payload = {
        "request_key": "register-generic-dataset-0001",
        "reviewer_identity": "Synthetic dataset reviewer",
        "note": "Register an external fixture without granting training authority",
    ...
    runtime_path = tmp_path / "generic-runtime.exe"
    runtime_path.write_bytes(b"synthetic runtime identity; never execute")
    executable_sha256 = hashlib.sha256(runtime_path.read_bytes()).hexdigest()
    runtime_sha256 = "a" * 64
    monkeypatch.setattr(
        backend,
    ...
    class ThreadingProbe:
        Thread = ThreadProbe

    monkeypatch.setattr(registry, "threading", ThreadingProbe)

    base = f"/v1/projects/{project_id}"
    with TestClient(app, client=("127.0.0.1", 50000)) as client:
        dataset = _sealed(
            client.post(
                base + "/vision-datasets",
                headers=OWNER_HEADERS,
                json=payload,
            ),
            201,
        ...
        runtime = _sealed(
            client.post(
                ...
                headers=OWNER_HEADERS,
                json={
                    "request_key": "register-generic-runtime-0001",
                    "reviewer_identity": "Synthetic runtime reviewer",
                    "note": "Register a test runtime identity without executing it",
        ...
        training = {
            "request_key": "generic-training-launch-0001",
            "reviewer_identity": "Synthetic training reviewer",
            "note": "Attempt to queue an unbound dataset through the default API",
        ...
        rejected = client.post(
            base + "/vision-training-runs",
            headers=OWNER_HEADERS,
            json=training,
        )
        seam_rejected = client.post(
            base + "/vision-training-runs",
            headers=OWNER_HEADERS,
            json=training | {"_test_only_allow_unbound_fixtures": True},
        )
        runs = _sealed(
            client.get(base + "/vision-training-runs", headers=OWNER_HEADERS)
        )

    assert dataset["provenance"] == "UNBOUND_EXTERNAL_FIXTURE"
    assert dataset["training_eligible"] is False
    assert dataset["production_release_allowed"] is False
    assert rejected.status_code == 409
    assert rejected.json()["error"]["code"] == "vision_model_hold"
    assert seam_rejected.status_code == 422
    assert seam_rejected.json()["error"]["code"] == "vision_request_invalid"
    assert pending == []
    assert runs["items"] == []
