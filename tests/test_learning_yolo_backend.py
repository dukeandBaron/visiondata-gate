...
def _dataset(root: Path) -> Path:
    ...
    for split, count in (("train", 4), ("val", 2), ("test", 2)):
        ...
        for index in range(count):
            ...
            ImageDraw.Draw(image).rectangle((16, 16, 47, 47), fill=(200, 100, index * 10))
    ...
    (root / "data.yaml").write_text(json.dumps({
        ...
        "test": "images/test", "names": ["synthetic_rectangle"],
    }), encoding="utf-8")
    return root


@pytest.mark.parametrize("values", [
    {"epochs": 0}, {"epochs": 6}, {"epochs": True}, {"imgsz": 65},
    {"imgsz": 640}, {"batch": 1}, {"threads": 5}, {"max_seconds": 601},
    {"seed": -1}, {"batch": 2.0}, {"max_seconds": float("nan")},
])
...
def test_small_budget_options_disable_minimum_warmup_and_step_every_batch(tmp_path):
    ...
    assert options["warmup_epochs"] == 0.0
    assert options["nbs"] == config.batch
    assert options["batch"] == config.batch
    assert options["device"] == "cpu"




@pytest.mark.parametrize("key", ["backbone", "head", "scales", "scale", "end2end", "reg_max"])
def test_initial_weights_architecture_cannot_rely_on_claimed_model_name(key):
    expected = {"backbone": [[-1, 1, "Conv", [64, 3, 2]]], "head": ["Detect"],
                "scales": {"n": [0.5, 0.25, 1024]}, "scale": "n",
                "end2end": True, "reg_max": 1}
...
def _box(class_id=0, xyxy=None, confidence=None):
    value = {"class_id": class_id, "xyxy": [0.25, 0.25, 0.75, 0.75] if xyxy is None else xyxy}
...
def test_validation_matching_class_aware_one_to_one_and_confidence_boundary():
    ...
    predictions = [_box(0, confidence=0.25), _box(0, confidence=0.24),
                   _box(0, confidence=0.8), _box(2, confidence=0.9)]
    ...
    assert result["matches"] == [{"prediction_index": 1, "ground_truth_index": 0, "iou": 1.0}]
    assert result["matched_ious"] == [1.0]
    assert result["reason_codes"] == ["FALSE_POSITIVE_CANDIDATE", "FALSE_NEGATIVE_CANDIDATE"]
...
def test_validation_matching_includes_exact_iou_boundary_and_excludes_below():
    ...
    exact = backend._match_validation_boxes([_box(xyxy=[0, 0, 1, 0.5], confidence=0.8)], truth)
    assert (exact["tp"], exact["fp"], exact["fn"]) == (1, 0, 0)
    assert exact["matched_ious"] == [0.5]
    below = backend._match_validation_boxes([_box(xyxy=[0, 0, 1, 0.499], confidence=0.8)], truth)
    assert (below["tp"], below["fp"], below["fn"]) == (0, 1, 1)


def test_validation_matching_global_greedy_iou_not_confidence_order():
    truth = [_box(xyxy=[0, 0, 1, 1])]
    predictions = [_box(xyxy=[0, 0, 1, 0.6], confidence=0.9),
...
def test_validation_matching_ties_and_empty_normal_images():
    tied = backend._match_validation_boxes([_box(confidence=0.8), _box(confidence=0.8)], [_box(), _box()])
    assert tied["matches"] == [
...
@pytest.mark.parametrize("invalid", [
    ...
    _box(class_id=True, confidence=0.8), _box(confidence=float("inf")),
])
...
class _FeedbackModel:
    ...
    def predict(self, **kwargs):
        ...
        return [SimpleNamespace(boxes=SimpleNamespace(
            ...
            conf=_CpuList([0.8]), cls=_CpuList([0.0]),
        ))]


def _feedback_request(dataset):
    return {"dataset": str(dataset), "dataset_receipt": backend._dataset_receipt(dataset),
            "source_dataset_sha256": "a" * 64, "runtime_sha256": "b" * 64}


def test_validation_feedback_predicts_every_val_member_only_and_filters_candidates(tmp_path):
    ...
    result = backend._validation_feedback(model, request, backend.YoloTrainingConfig(), "c" * 64)
    details = result["validation_samples_detail"]
    assert [row["sample_id"] for row in details] == ["0", "1"]
    assert [(row["tp"], row["fp"], row["fn"]) for row in details] == [(1, 0, 0), (0, 1, 0)]
    assert [row["sample_id"] for row in result["validation_feedback_candidates"]] == ["1"]
    ...
    for row in details:
        assert row["image_sha256"] == backend._sha(dataset / f"images/val/{row['sample_id']}.png")
        assert row["label_sha256"] == backend._sha(dataset / f"labels/val/{row['sample_id']}.txt")
...
def test_validation_feedback_rejects_bytes_changed_during_prediction(tmp_path):
    dataset = _dataset(tmp_path / "dataset")
    request = _feedback_request(dataset)
    model = _FeedbackModel(after_predict=lambda: (dataset / "labels/val/0.txt").write_text("", encoding="utf-8"))
    with pytest.raises(ValueError, match="YOLO_DATASET_CHANGED"):
        backend._validation_feedback(model, request, backend.YoloTrainingConfig(), "c" * 64)
...
def test_package_import_does_not_load_torch_or_ultralytics():
    ...
    assert subprocess.run([sys.executable, "-I", "-S", "-c", code],
...
def test_import_probe_uses_a_bounded_cold_start_budget(tmp_path, monkeypatch):
    observed = []
    ...
    def probe(
        ...
        *,
        ...
        observed.append(
            {
                ...
                "expected": expected_executable_sha256,
                "work_root": work_root,
        ...
        return {"status": "ready", "runtime_sha256": "a" * 64}

    monkeypatch.setattr(backend, "_probe_runtime", probe)
    result = backend.probe_vision_runtime(
        Path(sys.executable), "b" * 64, tmp_path, import_check=True
    )
...
@pytest.mark.parametrize("label", [
    "0 0.5 0.5 0.2 0.2 0.3 0.3", "1 0.5 0.5 0.2 0.2",
    "0 nan 0.5 0.2 0.2", "0 0.5 0.5 -0.2 0.2", "0 0 0 0.5 0.5",
])
...
@pytest.mark.parametrize("update", [
    {"download": "arbitrary code"}, {"train": "../private"},
    {"names": {1: "missing-zero"}}, {"nc": 2}, {"path": "https://example.com"},
])
...
def test_cancel_does_not_launch_and_persists_nonapproval(tmp_path, monkeypatch):
    ...
    result = backend.run_yolo_training(
        ...
        output_root=tmp_path / "job", config=backend.YoloTrainingConfig(), cancelled=lambda: True,
    )
    assert result["status"] == "cancelled"
    assert not result["production_approved"]
    assert "checkpoint" not in result
    assert json.loads((tmp_path / "job/result.json").read_text())["status"] == "cancelled"


def test_runtime_drift_prevents_training(tmp_path, monkeypatch):
    dataset = _dataset(tmp_path / "dataset")
    monkeypatch.setattr(backend, "_probe_runtime", lambda *_a, **_k: {
        "status": "ready", "runtime_sha256": "b" * 64,
    })
    monkeypatch.setattr(subprocess, "Popen", lambda *_a, **_k: pytest.fail("spawned"))
    with pytest.raises(ValueError, match="RUNTIME_FINGERPRINT_MISMATCH"):
        backend.run_yolo_training(
            executable=Path(sys.executable), expected_executable_sha256=backend._sha(Path(sys.executable)),
            expected_runtime_sha256="a" * 64, dataset_root=dataset,
            output_root=tmp_path / "job", config=backend.YoloTrainingConfig(),
        )


@pytest.mark.parametrize("interrupt", ["cancelled", "timed_out"])
def test_runtime_probe_interrupt_kills_only_its_owned_child(tmp_path, monkeypatch, interrupt):
    processes = []
    ...
    monkeypatch.setattr(backend, "_command", lambda executable, *_args: [
        str(executable), "-I", "-S", "-c", "import time; time.sleep(30)",
    ])
    result = backend._probe_runtime(
        Path(sys.executable), backend._sha(Path(sys.executable)), tmp_path,
        import_check=False, timeout=0.2,
        cancelled=(lambda: True) if interrupt == "cancelled" else None,
    )
    assert result["status"] == "unavailable"
    assert result["error_code"] == (
        "RUNTIME_PROBE_CANCELLED" if interrupt == "cancelled" else "RUNTIME_PROBE_TIMEOUT"
    )
    assert len(processes) == 1 and processes[0].poll() is not None


@pytest.mark.parametrize("interrupt", ["cancelled", "timed_out"])
def test_training_interrupt_is_durable_and_stops_worker(tmp_path, monkeypatch, interrupt):
    dataset = _dataset(tmp_path / "dataset")
    monkeypatch.setattr(backend, "_probe_runtime", lambda *_a, **_k: {
        "status": "ready", "runtime_sha256": "a" * 64,
    })
    processes = []
    original = subprocess.Popen
...
def test_explicit_actual_cpu_train_val_checkpoint(tmp_path):
    ...
    receipt = backend.probe_vision_runtime(executable, expected, root / "probe", import_check=True)
    ...
    result = backend.run_yolo_training(
        executable=executable, expected_executable_sha256=expected,
        expected_runtime_sha256=receipt["runtime_sha256"], dataset_root=dataset,
        output_root=root / "job", config=backend.YoloTrainingConfig(max_seconds=120),
    ...
    assert hashlib.sha256(checkpoint.read_bytes()).hexdigest() == result["checkpoint"]["sha256"]
    ...
    assert result["initialization"] == ("trusted_local_weights" if trusted_initial else "random")
