...
from visiondata_gate.learning_contracts import (
    CreateLearningCycle, FeedbackReview, ModelSelection, RunLearningRound,
)
...
def operation_records(tmp_path_factory):
    ...
    product = ProductService(tmp_path_factory.mktemp("operation-records") / "product", recover_interrupted=False)
    with pytest.MonkeyPatch.context() as environment:
        for name in ("VISIONDATA_SESSION_TOKEN", "VISIONDATA_DESKTOP_SESSION_TOKEN", "VISIONDATA_SESSION_ACTOR_USER_ID"):
        ...
        try:
            with TestClient(create_app(product)) as client:
                ...
                assert run["feedback"], "the fixture must exercise actual feedback review"
                feedback_requests = []
                for item in list(run["feedback"]):
                    ...
                    review = FeedbackReview(
                        ...
                        operator_attests_reviewed=True, classification="HARD_SAMPLE",
                        review_note="Generated masks reviewed; no automatic training ingestion",
                    )
                    run = service.review_feedback(ACTOR, cycle_id, run["run_id"], item["feedback_id"], review)
                    feedback_requests.append((item["feedback_id"], review))
                cycle = service.get_cycle(ACTOR, cycle_id)
                select = ModelSelection(
                    request_key="operation-select-0001",
                    expected_cycle_sha256=cycle["receipt_sha256"], expected_run_sha256=run["receipt_sha256"],
                    operator_attests_reviewed=True, action="APPROVE_SANDBOX",
                ...
                other = client.post("/v1/projects", headers=HEADERS, json={
                    "workspace_id": WORKSPACE, "name": "Separate lookup project",
                    "source_kind": "local_authorized_directory", "scenario_profile": "industrial",
                })
                assert other.status_code == 201
                yield {
                    "product": product, "service": service, "task_id": task_id,
                    "project_id": selected["project_id"], "other_project_id": other.json()["project_id"],
                    "cycle_id": cycle_id, "run_id": run["run_id"],
                    "original_cycle": original_cycle, "create": create, "train": train,
                    "feedback": feedback_requests[0], "select": select,
                }
        finally:
...
def lookup(records, operation, request_key, target_id, **overrides):
    return read_learning_operation(
        records["product"], overrides.get("actor", ACTOR),
        overrides.get("project_id", records["project_id"]), operation, request_key, target_id,
    )


@pytest.mark.parametrize("operation,target_field,result_kind", [
    ("create", "task_id", "cycle"), ("train", "cycle_id", "run"), ("select", "run_id", "cycle"),
])
def test_real_operations_return_server_request_digest_and_current_record(operation_records, operation, target_field, result_kind):
    ...
    current = records["service"].get_cycle(ACTOR, records["cycle_id"]) if result_kind == "cycle" else records["service"].get_run(ACTOR, records["run_id"])
    assert result["current_result"] == current
    assert result["result_receipt_sha256"] == current["receipt_sha256"]
    if operation == "create":
        assert result["current_result"]["receipt_sha256"] != records["original_cycle"]["receipt_sha256"]
...
def test_real_feedback_scope_is_feedback_id_not_cycle_id(operation_records):
    ...
    assert result["request_sha256"] == sha(request.model_dump(mode="json"))
    assert any(item["feedback_id"] == feedback_id and item["status"] == "TRIAGED_NOT_AUTO_INGESTED" for item in result["current_result"]["feedback"])
...
@pytest.mark.parametrize("operation", ["create", "train", "feedback", "select", "rollback", "cancel", "recover", "finalize", "followup"])
def test_unknown_request_never_proves_absence_or_allows_replay(operation_records, operation):
    result = lookup(operation_records, operation, "not-observed-request-001", "target-not-yet-remembered")
    verify_receipt(result)
...
def test_foreign_actor_and_foreign_project_do_not_leak_request_results(operation_records):
    records = operation_records
    with pytest.raises(NotFoundError):
        lookup(records, "create", records["create"].request_key, records["task_id"], actor="outsider")
    result = lookup(records, "create", records["create"].request_key, records["task_id"], project_id=records["other_project_id"])
    assert result["lookup_status"] == "NOT_FOUND"
    assert result["current_result"] is None
...
def test_pending_current_result_is_not_fabricated_as_completed(operation_records, status):
    records = operation_records
    ...
    try:
        with product.store._connection(immediate=True) as connection:
            service._put(connection, "cycle", original | {"status": status}, replace=True)
        result = lookup(records, "create", records["create"].request_key, records["task_id"])
        verify_receipt(result)
...
def test_tampered_current_record_is_rejected(operation_records):
    ...
    with product.store._connection() as connection:
        original = connection.execute("SELECT record_json FROM learning_records WHERE id=?", (records["cycle_id"],)).fetchone()[0]
    try:
        changed = json.loads(original)
        changed["status"] = "FINALIZED"
        with product.store._connection(immediate=True) as connection:
            connection.execute("UPDATE learning_records SET record_json=? WHERE id=?", (json.dumps(changed), records["cycle_id"]))
        ...
        with product.store._connection(immediate=True) as connection:
            connection.execute("UPDATE learning_records SET record_json=? WHERE id=?", (original, records["cycle_id"]))


@pytest.mark.parametrize("digest", ["not-a-digest", "0" * 64])
def test_invalid_or_mismatching_create_request_digest_is_rejected(operation_records, digest):
    records = operation_records
    request = records["create"]
    try:
        with records["product"].store._connection(immediate=True) as connection:
            connection.execute("UPDATE learning_requests SET request_sha=? WHERE scope=? AND operation=? AND actor=? AND request_key=?",
                (digest, records["task_id"], "create", ACTOR, request.request_key))
        with pytest.raises(LearningError, match="LEARNING_OPERATION_REQUEST_INTEGRITY_HOLD"):
            lookup(records, "create", request.request_key, records["task_id"])
    finally:
        with records["product"].store._connection(immediate=True) as connection:
            connection.execute("UPDATE learning_requests SET request_sha=? WHERE scope=? AND operation=? AND actor=? AND request_key=?",
                (sha(request.model_dump(mode="json")), records["task_id"], "create", ACTOR, request.request_key))


@pytest.mark.parametrize("operation,key,target", [
...
def test_lookup_identifiers_are_bounded_and_not_sql_fragments(operation_records, operation, key, target):
...
def test_project_authorization_happens_before_learning_storage_is_read():
    ...
    with pytest.raises(NotFoundError):
        read_learning_operation(Product(), "outsider", "unknown-project", "create", "unknown-request-001", "target")
    ...
    try:
        with TestClient(create_app(product)) as client:
            project = client.post("/v1/projects", headers=HEADERS, json={
                "workspace_id": WORKSPACE, "name": "Lookup without learning initialization",
                "source_kind": "local_authorized_directory", "scenario_profile": "industrial",
            }).json()
            result = read_learning_operation(product, ACTOR, project["project_id"], "create", "future-create-0001", "task-pending-freeze")
            verify_receipt(result)
            assert result["execution_status"] == "UNKNOWN_NOT_PROOF_OF_NO_WRITE"
            assert not (product.product_root / "learning").exists()
            with product.store._connection() as connection:
                assert connection.execute("SELECT COUNT(*) FROM sqlite_master WHERE name IN ('learning_requests','learning_records')").fetchone()[0] == 0
    finally:
        product.close(wait=True)
