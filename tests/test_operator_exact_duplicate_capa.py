"""Scoped exact-duplicate regressions transplanted from the authority tests."""
...
from visiondata_gate.compute_handoff import compute_preflight
...
def snapshot_client(tmp_path: Path) -> Iterator[tuple[TestClient, ProductService]]:
    expected_package = Path(__file__).resolve().parents[1] / "src" / "visiondata_gate"
    assert Path(capa_module.__file__).resolve().parent == expected_package
...
def _duplicate_snapshot_capa(
    ...
    *,
    annotate_one: bool = False,
    annotation_requirements: tuple[str, str] | None = None,
):
    project = client.post(
        "/v1/projects",
        headers=HEADERS,
    ...
    if annotation_requirements is None:
        ...
        samples = []
        for asset, requirement in zip(assets, annotation_requirements, strict=True):
            state_response = client.get(
                f"/v1/operator-workspaces/{WORKSPACE}/assets/"
                f"{asset['asset_id']}/annotations",
                headers=HEADERS,
            )
            ...
            samples.append(
                {
                    ...
                    "human_review": {
                        "reviewer_name": "Synthetic QA owner",
                        "note": "Reviewed identical image bytes and frozen annotation state.",
        ...
        frozen = client.post(
            ...
            json={
                ...
                "acceptance_requirements": {
                    ...
                    "purpose_description": "Review a same-policy duplicate sandbox dataset for compute.",
                    "category_vocabulary": ["unlabeled"],
                    "samples": samples,
                },
            },
        )
...
def test_exact_duplicate_capa_removes_only_derived_copy_and_child_rechecks(
    ...
    preflight = compute_preflight(service, ACTOR, executed.execution.child_task_id)
    assert preflight["eligibility"] == "HOLD"
    assert "EXPLICIT_ANNOTATION_REVIEW_REQUIRED" in preflight["blockers"]


def test_reviewed_v2_exact_duplicate_child_is_ready_for_offline_handoff(
    snapshot_client: tuple[TestClient, ProductService],
) -> None:
    client, service = snapshot_client
    task, source, plan, selected, _assets = _duplicate_snapshot_capa(
        client,
        service,
        annotation_requirements=("NOT_APPLICABLE", "NOT_APPLICABLE"),
    )
    assert source["data_profile"]["acceptance_requirements_sha256"]
    ...
    child_task_id = executed.execution.child_task_id
    child_gate = service.read_evidence_zip_json(
        ACTOR, child_task_id, "gate_result.json"
    )
    assert {
        trace["tool"] for trace in child_gate["tool_trace"] if trace["status"] == "ok"
    } == set(task.allowed_tools)
    preflight = compute_preflight(service, ACTOR, child_task_id)
    assert preflight["eligibility"] == "READY_FOR_OFFLINE_HANDOFF", preflight[
        "blockers"
    ]
    assert preflight["blockers"] == []
    assert preflight["production_release_allowed"] is False


def test_reviewed_v2_exact_duplicate_blocks_different_annotation_requirements(
    snapshot_client: tuple[TestClient, ProductService],
) -> None:
    client, service = snapshot_client
    task, _source, plan, selected, _assets = _duplicate_snapshot_capa(
        client,
        service,
        annotation_requirements=("OPTIONAL", "NOT_APPLICABLE"),
    )
    executed = _approve_and_execute_duplicate_capa(service, task, plan, selected)
    assert executed.derived_version is not None
    assert executed.derived_version.derived_image_count == 2
    assert executed.derived_version.operations[0].status == "BLOCKED"
    assert "逐样本标注要求" in executed.derived_version.operations[0].reason
    assert (
...
def test_exact_duplicate_exclusion_preserves_frozen_coverage_minimum(
    ...
    delivery = service.industrial_delivery_receipt(ACTOR, task.task_id)
    retained_ids, operations, unresolved = capa_module._operator_duplicate_exclusions(
...
def test_exact_duplicate_policy_rejects_unbound_or_unsupported_evidence(
    ...
    retained, operations, unresolved = capa_module._operator_duplicate_exclusions(
