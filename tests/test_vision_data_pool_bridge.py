...
def test_real_pool_http_registration_freezes_labels_and_restores_idempotent_result(
    ...
    with TestClient(app, client=("127.0.0.1", 50000)) as client:
        ...
        assert record["provenance"] == "GOVERNED_DATA_POOL_V1"
...
def test_governed_pool_dataset_enables_default_training_preflight(
    reviewed_pool, tmp_path, monkeypatch
) -> None:
    ...
    dataset = service.register_pool_dataset(
        ACTOR,
        project_id,
        registry.RegisterPoolDetectionDataset(**_request(projection, groups)),
    )
    runtime_path = tmp_path / "synthetic-python.exe"
    ...
    runtime = service.register_runtime(
        ...
        registry.RegisterVisionRuntime(
            request_key="bridge-runtime-register-0001",
            reviewer_identity="Synthetic runtime reviewer",
            note="Authorize a test-owned runtime identity without executing it",
    ...
    pending = []
    monkeypatch.setattr(
        registry.threading.Thread, "start", lambda thread: pending.append(thread)
    )

    capabilities = service.capabilities(ACTOR, project_id)
    run = service.create_training_run(
        ACTOR,
        project_id,
        registry.CreateVisionTrainingRun(
            request_key="bridge-training-launch-0001",
            reviewer_identity="Synthetic training reviewer",
            note="Queue governed detection training without executing the backend",
    ...
    assert capabilities["training_ready"] is True
    assert run["status"] == "QUEUED"
    assert run["production_release_allowed"] is False
