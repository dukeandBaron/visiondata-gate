...
@pytest.mark.parametrize(
    "fill,missing_metric", [(0, "false_negative_rate"), (1, "false_positive_rate")]
)
def test_missing_overall_positive_or_negative_denominator_is_none_and_hold(
    fill, missing_metric
):
    ...
    assert result["decision"] == "HOLD"
    assert any(
        code.startswith("AGGREGATE_METRIC_NOT_MEASURED") for code in result["blockers"]
    )
    if fill == 0:
...
def test_a_category_without_positives_holds_even_when_aggregate_is_measured():
    rows = [
        sample(),
        sample("sample-b", "background", mask=np.zeros((2, 4), dtype=np.uint8)),
    ]
    result = evaluate_models(model(10), model(), rows, EvaluationPolicy())
    assert result["aggregate"]["candidate"]["false_negative_rate"] == 0.0
    assert (
        result["categories"]["background"]["candidate"]["false_negative_rate"] is None
    )
    assert result["decision"] == "HOLD"
    assert any(
        code.startswith("CATEGORY_METRIC_NOT_MEASURED") for code in result["blockers"]
    )
...
def test_category_regression_holds_despite_good_aggregate_gain(monkeypatch):
    ...
    result = evaluate_models(model(10), model(), rows, EvaluationPolicy())
    assert (
        result["aggregate"]["candidate"]["dice"]
        > result["aggregate"]["baseline"]["dice"]
    )
...
def test_false_negative_and_false_positive_limits_are_independent(monkeypatch):
    ...
    result = evaluate_models(
        model(10), model(), [sample()], EvaluationPolicy(min_dice=0.5)
    )
...
def test_minimum_dice_and_required_gain_are_enforced(monkeypatch):
    ...
    result = evaluate_models(
        ...
        model(),
        [sample()],
        EvaluationPolicy(min_dice=0.95, min_dice_gain=0.99),
    )
...
def test_test_split_never_returns_sample_feedback_or_mining_tasks():
    result = evaluate_models(
        model(), model(10), [sample(split="test")], EvaluationPolicy(), split="test"
    )
...
def test_all_sample_splits_are_checked_before_any_prediction(monkeypatch):
    ...
    with pytest.raises(ValueError, match="SAMPLE_SPLIT_MISMATCH"):
        evaluate_models(
            model(10),
            model(),
            [sample(), sample("train-row", split="train")],
            EvaluationPolicy(),
        )
...
def test_empty_or_duplicate_sample_identity_is_rejected(rows):
    with pytest.raises(
        ValueError,
        match="EVALUATION_SAMPLES_EMPTY|SAMPLE_ID_INVALID|SAMPLE_ID_DUPLICATE",
    ):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("max_false_positive_rate", 1.1),
        ("min_dice", 1.1),
        ("min_dice_gain", -0.1),
        ("max_category_dice_regression", -0.1),
        ("max_p95_latency_ms", 0),
        ("max_p95_latency_ms", 60001),
...
def test_prediction_digest_changes_when_probabilities_change_without_mask_change(
    monkeypatch,
):
    truth = sample().mask.astype(float)
    ...
    assert (
        left["candidate"]["prediction_mask_sha256"]
        == right["candidate"]["prediction_mask_sha256"]
    )
    assert (
        left["candidate"]["prediction_sha256"]
        != right["candidate"]["prediction_sha256"]
    )
