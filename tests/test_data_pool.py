...
def test_pool_request_key_conflict_fails_without_second_version(learning_input) -> None:
    ...
    changed = request.model_copy(
        update={"review_note": "A different review payload is not a retry"}
    )

    with pytest.raises(DataPoolError, match="IDEMPOTENCY_CONFLICT"):
        service.create_pool(ACTOR, task_id, changed)
    assert (
        service.get_pool(ACTOR, first["pool"]["pool_id"])["pool"]["version_ids"]
...
def test_qualified_subset_requires_new_gate_before_bridge_can_consume_it(
    ...
    projection = service.create_pool(ACTOR, task_id, create)
    member_payloads = [item.model_dump(mode="json") for item in create.members]
    readiness_members = {
        item["sample_id"]: item
    ...
    projection = service.create_version(ACTOR, projection["pool"]["pool_id"], version)
    ...
    final = service.create_version(ACTOR, updated["pool"]["pool_id"], next_version)
    assert final["pool"]["origin_task_id"] == task_id
    assert final["pool"]["current_task_id"] == child.task_id
    assert (
        final["current_version"]["parent_version_id"]
...
def test_reviewed_empty_annotation_and_clean_member_complement_can_be_candidates(
    ...
    task_id, _preflight, _groups = make_task(client, product, include_normal=True)
    ...
    with pytest.raises(DataPoolError, match="QUALIFICATION_REQUIRES_MEMBER_EVIDENCE"):
        DataPoolService(product).create_pool(ACTOR, second_task, invalid_complement)
