...
def test_runtime_receipt_requires_hash_bound_raw_evidence(tmp_path) -> None:
    ...
    team_path.write_text('{"phase":"Active"}', encoding="utf-8")
    event_path.write_text('{"event_id":"$event"}', encoding="utf-8")
    skills_path.write_text("[]", encoding="utf-8")

    workers = [
        "visiondata-release-lead",
        "visiondata-image-quality",
        "visiondata-duplicate-leakage",
        "visiondata-annotation-integrity",
        "visiondata-coverage-matrix",
        "visiondata-ai-council",
        "visiondata-policy-judge",
        "visiondata-repair-operator",
        "visiondata-audit-clerk",
    ]
    receipt = {
        ...
        "skill_assignments": [
            {"worker": worker, "skills": ["bound-skill"]} for worker in workers
        ],
        "raw_evidence": {
            "team_status": {
