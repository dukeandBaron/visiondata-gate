...
test('continual sandbox selection requires one bound retention receipt', async () => {
  const validate = validator('validateLearningRun');
  const value = await run(); const good = evaluation();
  const perfect = metrics(2, 0, 2, 0);
  const sample = structuredClone(good.sample_results[0]);
  sample.candidate = { ...perfect, prediction_sha256: sha('d'), prediction_mask_sha256: sha('e') }; sample.error_candidate = false;
  good.aggregate.candidate = perfect; good.categories.part.candidate = perfect;
  good.sample_results = [sample]; good.decision = 'ELIGIBLE'; good.blockers = [];
  value.evaluation = good; value.evaluation_sha256 = await digest(good); value.feedback = [];
  value.selection = { action: 'APPROVE_SANDBOX_CONTINUAL', actor: 'reviewer', note: 'Approve only after retention review', evaluation_sha256: value.evaluation_sha256, continual_retention_receipt_sha256: sha('c'), at: '2026-09-12T12:00:02.000+00:00' };
  assert.equal((await validate(await sealed(value), scope, 'cycle_a')).selection.continual_retention_receipt_sha256, sha('c'));
  const missing = structuredClone(value); delete missing.selection.continual_retention_receipt_sha256;
  await assert.rejects(validate(await sealed(missing), scope, 'cycle_a'), hold);
  const dangling = structuredClone(value); dangling.selection.action = 'APPROVE_SANDBOX';
  await assert.rejects(validate(await sealed(dangling), scope, 'cycle_a'), hold);
});
...
test('write requests retain request_key and revision hashes without adding authority or defaults', () => {
  ...
  assert.equal(selection({ ...actionRequest, expected_run_sha256: sha('c'), expected_continual_retention_receipt_sha256: sha('d'), action: 'APPROVE_SANDBOX_CONTINUAL' }).expected_continual_retention_receipt_sha256, sha('d'));
  assert.throws(() => selection({ ...actionRequest, expected_run_sha256: sha('c'), action: 'APPROVE_SANDBOX_CONTINUAL' }), hold);
  assert.throws(() => selection({ ...actionRequest, expected_run_sha256: sha('c'), expected_continual_retention_receipt_sha256: sha('d'), action: 'APPROVE_SANDBOX' }), hold);
