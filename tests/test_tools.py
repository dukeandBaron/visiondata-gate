...
def test_shared_batch_fingerprint_preserves_binding_and_fails_closed_on_drift(
    tmp_path: Path,
) -> None:
    paths = generate_demo_dataset(tmp_path / "dataset", seed=41)
    manifest = _read(paths["batch_manifest"], BatchManifest)
    contract = BatchContract()
    fingerprint = build_batch_fingerprint(paths["batch_root"], manifest)

    direct = execute_tool_gateway(
        "coverage_matrix", paths["batch_root"], manifest, contract
    )
    shared = execute_tool_gateway(
        "coverage_matrix",
        paths["batch_root"],
        manifest,
        contract,
    ...
    image_path = paths["batch_root"] / manifest.samples[0].relative_path
    image_path.write_bytes(image_path.read_bytes() + b"drift")
    stale = execute_tool_gateway(
        "coverage_matrix",
        paths["batch_root"],
        manifest,
        contract,
    ...
    assert stale[0] == []
    assert stale[2] == {}
    assert stale[1].status == "error"
    assert "request digest mismatch" in (stale[1].error or "")
